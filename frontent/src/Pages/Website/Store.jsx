...
const Store = () => {
    ...
    }, [fetchCategory, fetchColor, searchParams]);
    ...
    }, [limit, selColor, setSearchParams]);

    useEffect(() => {
        fetchProduct(limit, selColor, category_slug);
        updateSearchParams();
    }, [limit, selColor, category_slug, fetchProduct, updateSearchParams]);
                ...
                <div>
                    <div className='bg-[#F6F7F8] rounded-lg py-4 pl-5'>
                        ...
                        <ul className='mt-2'>
                            ...
                            <Link to={"/store"}>
                                <li className='m-1 cursor-pointer' style={{ fontWeight: category_slug == null ? 'bold' : '' }}>All</li>
                            </Link>
                            {category.map((cat, i) => (
                                <Link key={i} to={"/store/" + cat.slug} style={{ fontWeight: category_slug === cat.slug ? 'bold' : '' }}>
                                    <li className='m-1 cursor-pointer'>{cat.name}</li>
                                </Link>
                            ))}
                        </ul>
                    </div>
                        ...
                        <ul>
                            {colors.map((color, i) => (
                                ...
                                <li key={i} className='mt-2 cursor-pointer' style={{ fontWeight: selColor === color._id ? 'bold' : '' }} onClick={() => setSelColor(color._id)}>
                                    <span className='px-2.5 rounded-full m-2' style={{ background: color.code }}></span>
                                    {color.name}
                                </li>
                            ))}
                        </ul>
